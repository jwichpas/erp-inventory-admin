<template>
  <div class="space-y-4">
        ...
        <div>
          ...
          <div class="mt-1 relative">
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Número, documento, almacén..."
    ...
    <div v-else-if="receptions.length > 0" class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="reception in receptions" :key="reception.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <Package class="h-5 w-5 text-gray-400 mr-3" />
                  <div>
                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                      {{ reception.reception_number || `REC-${reception.id.slice(-8).toUpperCase()}` }}
                    </div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                      {{ reception.id.slice(0, 8) }}...
                    </div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <Warehouse class="h-4 w-4 text-gray-400 mr-2" />
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end space-x-2">
                  ...
                  <button
                    v-if="canEdit(reception)"
                    @click="$emit('edit', reception.id)"
                  ...
                  <button
                    v-if="canApprove(reception)"
                    @click="$emit('approve', reception.id)"
                    class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300"
                    title="Aprobar recepción"
                  >
                    <Check class="h-4 w-4" />
                  </button>
                  
                  <button
                    v-if="canReject(reception)"
                    @click="$emit('reject', reception.id)"
                  ...
                  <div class="relative" v-if="hasAdditionalActions(reception)">
                    <button
                      @click="toggleActionMenu(reception.id)"
    ...
    <div v-else class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      <div class="text-center">
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Comience creando una nueva recepción de mercancía.
        </p>
        <div class="mt-6">
          <button
            @click="$emit('create')"
...
import type { ReceptionListItem, ReceptionFilters, PaginationInfo } from '../../types'
...
defineEmits<{
  ...
  approve: [id: string]
  reject: [id: string]
  filter: [filters: Partial<ReceptionFilters>]
  page: [page: number]
}>()
...
const canEdit = (reception: ReceptionListItem) => {
  return reception.status !== 'COMPLETED' && reception.status !== 'CANCELLED'
}

const canApprove = (reception: ReceptionListItem) => {
  return reception.status === 'PENDING' || reception.status === 'PARTIAL'
}

const canReject = (reception: ReceptionListItem) => {
  return reception.status !== 'COMPLETED' && reception.status !== 'CANCELLED'
}

const hasAdditionalActions = (reception: ReceptionListItem) => {
...
watch(() => props.filters, (newFilters) => {
  ...
  warehouseFilter.value = newFilters.warehouse_id || ''
